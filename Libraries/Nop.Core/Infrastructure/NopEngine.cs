...
namespace Nop.Core.Infrastructure
{
    ...
    public class NopEngine : IEngine
    {
        ...
        /// <summary>
        /// Register services using native asp.net core DI
        /// </summary>
        /// <param name="nopConfig">Startup Nop configuration parameters</param>
        /// <param name="services">Collection of service descriptors</param>
        /// <param name="typeFinder">Type finder</param>
        protected virtual IServiceProvider RegisterServices(NopConfig nopConfig, IServiceCollection services, ITypeFinder typeFinder)
        {
            services.AddSingleton<IEngine>(this);
            services.AddSingleton<ITypeFinder>(typeFinder);

            //find dependency registrars provided by other assemblies
            var serviceRegistrars = typeFinder.FindClassesOfType<IServiceRegistrar>();

            // changed
            //create and sort instances of dependency registrars
            var instances = serviceRegistrars
                //.Where(dependencyRegistrar => PluginManager.FindPlugin(dependencyRegistrar)?.Installed ?? true) //ignore not installed plugins
                .Select(dependencyRegistrar => (IServiceRegistrar)Activator.CreateInstance(dependencyRegistrar))
                .OrderBy(dependencyRegistrar => dependencyRegistrar.Order);

            //register all provided dependencies
            foreach (var serviceRegistrar in instances)
                serviceRegistrar.Register(services, null, typeFinder, nopConfig);
