    ...
    internal static class RouteTableFactory
    {
        ...
        internal static RouteTable Create(IEnumerable<Type> componentTypes)
        {
            ...
            foreach (var componentType in componentTypes)
            {
                ...
                var templates = routeAttributes.SelectMany(t => TryGetGenericTemplates(t)).ToArray();
                templatesByHandler.Add(componentType, templates);
            }
            return Create(templatesByHandler);
        }

        // ADDED slugs to a RouteTable
        /// <summary>
        /// Get slugs from IUrlRecordService whihc contains slugs
        /// </summary>
        /// <param name="routeAttribute"></param>
        /// <returns></returns>
        private static IEnumerable<string> TryGetGenericTemplates(RouteAttribute routeAttribute)
        {
            if (routeAttribute.Template.EndsWith(":urlservice}"))
            {
                var urlService = EngineContext.Current.Resolve<IUrlRecordService>();
                var generic = routeAttribute.Template.TrimStart('/').TrimStart('{').TrimEnd('}').Split(':')[0];

                // if cash is used here we'll get a strange thing: incomplete urlrecords and an error of the comparer (constraint conflict) 
                // I have done a workaround in RouteComparison
                return urlService.GetAllUrlRecordsCached()//.GetAllUrlRecords()//
                    .Where(x => x.IsActive && x.EntityName.Equals(generic, StringComparison.InvariantCultureIgnoreCase))
                    //.Select(x => "/" + WebUtility.UrlEncode(x.Slug));
                    .Select(x => "/" + x.Slug);
            }

            return new[] { routeAttribute.Template };
        }
        ...
        {
            // WORKAROUND probably it is a bug of the Comparer for urlService.GetAllUrlRecordsCached()
            if (x.Handler.Equals(y.Handler) && x.Template.Equals(y.Template))
            {
                return -1;
            }
