...
namespace spaCommerce.Models.Catalog
{
    ...
    /// </summary>
    public partial class CatalogPagingFilteringModel : Nop.Web.Framework.UI.Paging.CatalogPagingFilteringModel
    {
        #region Ctor
        ...
        public partial class SpecificationFilterModel : BaseNopModel
        {
            ...
            public virtual void PrepareSpecsFilters(IList<int> alreadyFilteredSpecOptionIds,
                ...
                IWebHelper webHelper, IWorkContext workContext, ICacheManager cacheManager, NavigationManager uriHelper)
            {
                Enabled = false;
                ...
                RemoveFilterUrl = uriHelper.GetRelativePath() + uriHelper.RemoveQueryString(new[] { QUERYSTRINGPARAM });
                ...
                NotFilteredItems = allFilters.Except(alreadyFilteredOptions).Select(x =>
                {
                    ...
                    //CHANGED
                    var filterUrl = uriHelper.GetRelativePath() + uriHelper.ModifyQueryString(new Dictionary<string, StringValues>()
                    {
                        { QUERYSTRINGPARAM, new StringValues(alreadyFiltered.OrderBy(id => id).Select(id => id.ToString()).ToArray()) }
                    });
