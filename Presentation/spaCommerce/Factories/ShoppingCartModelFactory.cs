...
namespace spaCommerce.Factories
{
    ...
    public partial class ShoppingCartModelFactory : IShoppingCartModelFactory
    {
        ...
        protected virtual CheckoutAttributeList PrepareCheckoutAttributeModels(IList<ShoppingCartItem> cart)
        {
            if (cart == null)
                throw new ArgumentNullException(nameof(cart));

            var model = new CheckoutAttributeList();
        ...
        public virtual ShoppingCartModel.ShoppingCartItemModel UpdateQuantityShoppingCartItem(IList<ShoppingCartItem> cart, ShoppingCartModel.ShoppingCartItemModel item)
        {
            if (cart == null)
                throw new ArgumentNullException(nameof(cart));
            ...
            {
                item.UnitPrice = _localizationService.GetResource("Products.CallForPrice");
            }
            else
            {
                ...
                item.UnitPrice = _priceFormatter.FormatPrice(shoppingCartUnitPriceWithDiscount);
            }
            //subtotal, discount
            if (sci.Product.CallForPrice &&
                //also check whether the current user is impersonated
                (!_orderSettings.AllowAdminsToBuyCallForPriceProducts || _workContext.OriginalCustomerIfImpersonated == null))
            {
                item.SubTotal = _localizationService.GetResource("Products.CallForPrice");
            }
            else
            {
                //sub total
                ...
                item.SubTotal = _priceFormatter.FormatPrice(shoppingCartItemSubTotalWithDiscount);
                item.MaximumDiscountedQty = maximumDiscountQty;
                ...
                if (shoppingCartItemDiscountBase > decimal.Zero)
                {
                    ...
                    if (shoppingCartItemDiscountBase > decimal.Zero)
                    {
                        ...
                        item.Discount = _priceFormatter.FormatPrice(shoppingCartItemDiscount);
                    }
                }
            }
        ...
        /// <param name="sci">Shopping cart item</param>
        /// <returns>Shopping cart item model</returns>
        public virtual WishlistModel.ShoppingCartItemModel PrepareWishlistItemModel(IList<ShoppingCartItem> cart, ShoppingCartItem sci)
        {
            if (cart == null)
                throw new ArgumentNullException(nameof(cart));
            ...
            bool prepareAndDisplayOrderReviewData = false)
        {
            ...
            foreach (var pm in buttonPaymentMethods)
            {
                ...
                var viewComponentName = pm.GetPublicViewComponentType();
                model.ButtonPaymentMethodViewComponentTypes.Add(viewComponentName);
            }
            //hide "Checkout" button if we have only "Button" payment methods
            model.HideCheckoutButton = !nonButtonPaymentMethods.Any() && model.ButtonPaymentMethodViewComponentTypes.Any();
