...
namespace spaCommerce.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        private readonly IShoppingCartComponentService _shoppingCartComponentService;
        ...
        public ProductModelFactory(CaptchaSettings captchaSettings,
            ...
            IShoppingCartComponentService shoppingCartComponentService)
        {
            ...
            this._shoppingCartComponentService = shoppingCartComponentService;
        }
        ...
        // CHANGED access level from protected to public
        /// <summary>
        /// Prepare the product breadcrumb model
        /// </summary>
        /// <param name="product">Product</param>
        /// <returns>Product breadcrumb model</returns>
        public virtual ProductDetailsModel.ProductBreadcrumbModel PrepareProductBreadcrumbModel(Product product)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
        ...
        ///// <returns></returns>
        //public virtual ProductDetailsModel.ProductPriceModel PrepareProductPriceModel(ProductDetailsModel productDetailsModel)
        //{
        //    if (productDetailsModel == null) return null;

        //    var product = _productService.GetProductById(productDetailsModel.Id);

        //    if (product == null) return null;

        //    var priceModel = PrepareProductPriceModel(product);

        //    priceModel.Price = string.IsNullOrEmpty(productDetailsModel.ProductAttributes.Price)
        //        ? priceModel.Price : productDetailsModel.ProductAttributes.Price;

        //    priceModel.PriceValue = productDetailsModel.ProductAttributes.BasePrice == 0
        //        ? priceModel.PriceValue : productDetailsModel.ProductAttributes.BasePrice;

        //    priceModel.BasePricePAngV = string.IsNullOrEmpty(productDetailsModel.ProductAttributes.BasePricePAngV)
        //        ? priceModel.BasePricePAngV : productDetailsModel.ProductAttributes.BasePricePAngV;
        ...
        public virtual ProductDetailsModel.ProductPriceModel PrepareProductPriceModel(Product product)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
        ...
        public virtual ProductAttributeList PrepareProductAttributeModels(Product product, ShoppingCartItem updatecartitem = null, IDictionary<string, string> form = null)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            ...
            var model = new ProductAttributeList();
            ...
            //added
            if (form != null)
                model.AttributeFormCollection = form;

            return model;
        }

        private void InitFormCollection(ProductAttributeList attributeList)
        {
            var form = attributeList.AttributeFormCollection;// = new Dictionary<string, string>();
            ...
            foreach (var attr in attributeList)
            {
                ...
                switch (attr.AttributeControlType)
                {
                    case AttributeControlType.DropdownList:
                        {
                            form[controlId] = attr.Values.FirstOrDefault(x => x.IsPreSelected)?.Id.ToString() ?? (attr.IsRequired ? attr.Values.FirstOrDefault()?.Id.ToString() : "0");
                            foreach (var attrValue in attr.Values)
                                if (attrValue.CustomerEntersQty)
                                    form[$"{controlId}_{attrValue.Id}_qty"] = attrValue.Quantity.ToString();
                        }
                        break;
                    case AttributeControlType.ImageSquares:
                    case AttributeControlType.ColorSquares:
        ...
        private ProductAttributeList HandleAttributeList(ProductAttributeList attributeList, bool validateAttributeConditions = true, bool loadPicture = true)
        {
            ...
            int productId = attributeList.FirstOrDefault()?.ProductId ?? 0;
            ...
            var attributeXml = _shoppingCartComponentService.ParseProductAttributes(product, attributeList.Form, errors);
            ...
            if (product.IsRental)
            {
                _shoppingCartComponentService.ParseRentalDates(product, attributeList.Form, out rentalStartDate, out rentalEndDate);
            }
            ...
            //base price
            var basepricepangv = "";
            decimal finalPriceWithDiscountBase = default;
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices) && !product.CustomerEntersPrice)
            {
                ...
                finalPriceWithDiscountBase = _taxService.GetProductPrice(product, finalPrice, out decimal _);
                var finalPriceWithDiscount = _currencyService.ConvertFromPrimaryStoreCurrency(finalPriceWithDiscountBase, _workContext.WorkingCurrency);
            ...
            string pictureFullSizeUrl = null;// = string.Empty;
            string pictureDefaultSizeUrl = null;// = string.Empty;
            if (loadPicture)
            {
            ...
            attributeList.Gtin = gtin;
            attributeList.ManufacturerPartNumber = mpn;
            attributeList.Sku = sku;
            attributeList.BasePrice = finalPriceWithDiscountBase;
            attributeList.Price = price;
            attributeList.BasePricePAngV = basepricepangv;
            attributeList.StockAvailability = stockAvailability;
            attributeList.EnabledAttributeMappingIds = enabledAttributeMappingIds.ToArray();
            attributeList.DisabledAttributeMappingIds = disabledAttributeMappingIds.ToArray();
            attributeList.FullSizeImageUrl = pictureFullSizeUrl;
            attributeList.ImageUrl = pictureDefaultSizeUrl;
            attributeList.IsFreeShipping = isFreeShipping;
            attributeList.Message = errors;

            return attributeList;
        }

        public ProductDetailsModel AdjustProductDetailsModel(ProductDetailsModel model)
        {
            if (model == null)
                throw new Exception("ProductDetailsModel is null");

            if (!model.ProductAttributes.Any())
                return model;

            HandleAttributeList(model.ProductAttributes);

            model.Gtin = model.ProductAttributes.Gtin;
            model.ManufacturerPartNumber = model.ProductAttributes.ManufacturerPartNumber;
            model.Sku = model.ProductAttributes.Sku;
            model.ProductPrice.Price = model.ProductAttributes.Price;
            model.ProductPrice.PriceValue = model.ProductAttributes.BasePrice;
            model.ProductPrice.BasePricePAngV = model.ProductAttributes.BasePricePAngV;
            model.StockAvailability = model.ProductAttributes.StockAvailability;

            model.DefaultPictureModel.FullSizeImageUrl = string.IsNullOrEmpty(model.ProductAttributes.FullSizeImageUrl)
                ? model.DefaultPictureModel.FullSizeImageUrl : model.ProductAttributes.FullSizeImageUrl;

            model.DefaultPictureModel.ImageUrl = string.IsNullOrEmpty(model.ProductAttributes.ImageUrl)
                ? model.DefaultPictureModel.ImageUrl : model.ProductAttributes.ImageUrl;

            model.IsFreeShipping = model.ProductAttributes.IsFreeShipping;
        ...
        protected virtual PictureModel PrepareProductDetailsPictureModel(Product product, bool isAssociatedProduct, out IList<PictureModel> allPictureModels)
        {
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                var attributePictures = product.ProductAttributeMappings.SelectMany(x => x.ProductAttributeValues.Select(y => y.PictureId));
                foreach (var picture in pictures)
                    // we get only pictures which aren't contained in attribute values, because ignored pictures will be showed when an attribute is picked out 
                    if (attributePictures.All(x => picture.Id != x))
                    {
                        var pictureModel = new PictureModel
                        {
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            //model.ProductPrice = PrepareProductPriceModel(model);
            ...
            // we adjust the product detail model according products attributes.
            //return model.ProductAttributes.InitAttributeList().AdjustProductDetailsModel(model);


            return AdjustProductDetailsModel(model); ;
        }
